...
const ItemEdit: NextPage = (props) => {
  ...
  const itemQuery = trpc.useQuery(["items.getItem", { id: router.query.id!.toString(), readOnly: false }], {
    onError: (e) => {
      toast.error(getTypedFormattedString(intl, "alert-code-not-found-edit"));
      router.push("/");
    },
    onSuccess: (data) => {
      if (data.binary) {
      ...
      reset({
        ...
        title: data.title,
        public: data.public ? "true" : undefined,
      });
    },
    cacheTime: 0,
  });
  ...
  const createMutation = trpc.useMutation("items.createOrUpdate", {
    onSuccess(data) {
      ...
      queryUtils.invalidateQueries(["items.getItem", { id: router.query.id!.toString(), readOnly: false }]);
      queryUtils.invalidateQueries(["items.getItem", { id: router.query.id!.toString(), readOnly: true }]);
  ...
  if (itemQuery.data && !itemQuery.isError && !itemQuery.data.binary) {
    return (
      <div>
        <h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <Link href={`/items/${router.query.id!.toString()}`}>
